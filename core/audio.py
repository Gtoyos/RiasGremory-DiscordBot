...
class Audio:
    ...
    async def notify(self, ctx):
        """Toggle song announcement and other Rias messages."""
    ...
    async def vote(self, ctx, percent: int):
        ...
        if percent == 0:
            enabled = False
            await self._embed_msg(
                ctx, "Voting disabled. All users can use queue management commands. Anarchism rules!"
            )
        else:
            enabled = True
    ...
    async def status(self, ctx):
        """(Gtoyos Command). Enables/disables songs' titles as status."""
        status = await self.config.status()
    ...
    async def bump(self, ctx, index: int):
        ...
        if not self._player_check(ctx):
            return await self._embed_msg(ctx, "Nothing playing. (-_-;)・・・")
        ...
        if dj_enabled:
            if not await self._can_instaskip(ctx, ctx.author):
                return await self._embed_msg(ctx, "You need the DJ role to bump songs. (*μ_μ)")
        if index > len(player.queue) or index < 1:
            return await self._embed_msg(
                ctx, "Song number must be greater than 1 and within the queue limit, baka."
            )
    ...
    async def disconnect(self, ctx):
        ...
        if self._player_check(ctx):
            if dj_enabled:
                if not await self._can_instaskip(ctx, ctx.author):
                    return await self._embed_msg(ctx, "You need the DJ role to disconnect me, baka.")
            if (
                not await self._can_instaskip(ctx, ctx.author)
                and not await self._is_alone(ctx, ctx.author)
            ):
                return await self._embed_msg(ctx, "There are other people listening to music. Don't be a bad person :( )")
            else:
    ...
    async def pause(self, ctx):
        ...
        if (
            ...
            return await self._embed_msg(
                ctx, "You must be in the voice channel to pause the music, baka."
            )
        if dj_enabled:
            if (
                not await self._can_instaskip(ctx, ctx.author)
                ...
                return await self._embed_msg(ctx, "You need the DJ role to pause songs, baka.")
        ...
        if player.paused and command == "pause":
            return await self._embed_msg(ctx, "Track is paused. (」°ロ°)」")
        if player.current and command == "resume":
            return await self._embed_msg(ctx, "Track is playing. (⌒‿⌒)")
    ...
    async def percent(self, ctx):
        """Queue percentage."""
        if not self._player_check(ctx):
            return await self._embed_msg(ctx, "Nothing playing. (T_T)")
        player = lavalink.get_player(ctx.guild.id)
        ...
        try:
            ...
            return await self._embed_msg(ctx, "Nothing in the queue.  〣( ºΔº )〣")
    ...
    async def play(self, ctx, *, query):
        ...
        if not self._player_check(ctx):
            try:
                ...
                return await self._embed_msg(ctx, "Connect to a voice channel first, baka.")
        if dj_enabled:
            if not await self._can_instaskip(ctx, ctx.author):
                return await self._embed_msg(ctx, "You need the DJ role to queue songs, baka.")
        player = lavalink.get_player(ctx.guild.id)
        ...
        if (
            ...
            return await self._embed_msg(
                ctx, "You must be in the voice channel to use the play command, baka"
            )
        ...
        if not tracks:
            return await self._embed_msg(ctx, "Nothing found. (•ิ_•ิ)?")
        ...
        if "list" in query and "ytsearch:" not in query:
            ...
            embed = discord.Embed(
                colour=ctx.guild.me.top_role.colour,
                title="Playlist Enqueued (ﾉ◕ヮ◕)ﾉ*:･ﾟ✧",
    ...
    async def _playlist_append(self, ctx, playlist_name, *url):
        ...
        async with self.config.guild(ctx.guild).playlists() as playlists:
            try:
                ...
                return await self._embed_msg(ctx, "No playlist with that name, are you dyslexic?")
    ...
    async def _playlist_create(self, ctx, playlist_name):
        ...
        async with self.config.guild(ctx.guild).playlists() as playlists:
            if playlist_name in playlists:
                return await self._embed_msg(
                    ctx, "Playlist name already exists, try again with a different name. Be more creative!"
                )
        ...
        async with self.config.guild(ctx.guild).playlists() as playlists:
            try:
                if (
                    ...
                    return await self._embed_msg(ctx, "You are not the author of that playlist, baka.")
    ...
    async def _playlist_queue(self, ctx, playlist_name=None):
        ...
        if dj_enabled:
            if not await self._can_instaskip(ctx, ctx.author):
                return await self._embed_msg(ctx, "You need the DJ role to save playlists, baka")
        async with self.config.guild(ctx.guild).playlists() as playlists:
            if playlist_name in playlists:
                return await self._embed_msg(
            ...
            if not self._player_check(ctx):
                return await self._embed_msg(ctx, "Nothing playing. ┐(￣ヘ￣)┌")
        player = lavalink.get_player(ctx.guild.id)
        tracklist = []
        ...
        if not playlist_name:
            await self._embed_msg(ctx, "Please enter a name for this playlist. \(^ヮ^)/")
    ...
    async def _playlist_start(self, ctx, playlist_name=None):
        ...
        try:
            ...
            await self._embed_msg(ctx, "That playlist doesn't exist. ( ; ω ; )")

    @checks.is_owner()
    @playlist.command(name="upload")
    async def _playlist_upload(self, ctx):
        """(Gtoyos Command). Convert a Red v2 playlist file to a playlist."""
        if not await self._playlist_check(ctx):
            return
        player = lavalink.get_player(ctx.guild.id)
        await self._embed_msg(
    ...
    async def _playlist_check(self, ctx):
        ...
        if dj_enabled:
            if not await self._can_instaskip(ctx, ctx.author):
                await self._embed_msg(ctx, "You need the DJ role to use playlists, baka.")
                return False
        if not self._player_check(ctx):
            try:
                await lavalink.connect(ctx.author.voice.channel)
                ...
                await self._embed_msg(ctx, "Connect to a voice channel first, baka.")
                return False
        player = lavalink.get_player(ctx.guild.id)
        player.store("channel", ctx.channel.id)
        player.store("guild", ctx.guild.id)
        if (
            ...
            await self._embed_msg(
                ctx, "You must be in the voice channel to use the playlist command, baka."
            )
            return False
        if not await self._currency_check(ctx, jukebox_price):
            return False
    ...
    async def prev(self, ctx):
        ...
        if not self._player_check(ctx):
            return await self._embed_msg(ctx, "Nothing playing. (・・;)ゞ")
        dj_enabled = await self.config.guild(ctx.guild).dj_enabled()
        player = lavalink.get_player(ctx.guild.id)
        shuffle = await self.config.guild(ctx.guild).shuffle()
        if dj_enabled:
            if (
                ...
                return await self._embed_msg(ctx, "You need the DJ role to skip songs, baka")
        if (
            (not ctx.author.voice or ctx.author.voice.channel != player.channel)
            and not await self._can_instaskip(ctx, ctx.author)
        ):
            return await self._embed_msg(
                ctx, "You must be in the voice channel to skip the music, baka"
            )
        if shuffle:
    ...
    async def queue(self, ctx, page: int = 1):
        """Lists the queue."""
        if not self._player_check(ctx):
            return await self._embed_msg(ctx, "There's nothing in the queue. Add sth!")
        player = lavalink.get_player(ctx.guild.id)
        if not player.queue:
            return await self._embed_msg(ctx, "There's nothing in the queue. Add sth!")
    ...
    async def repeat(self, ctx):
        ...
        if dj_enabled:
            if (
                ...
                return await self._embed_msg(ctx, "You need the DJ role to toggle repeat, baka")
        repeat = await self.config.guild(ctx.guild).repeat()
        ...
        if self._player_check(ctx):
            ...
            if (
                ...
                return await self._embed_msg(
                    ctx, "You must be in the voice channel to toggle repeat, baka"
                )
    ...
    async def remove(self, ctx, index: int):
        ...
        player = lavalink.get_player(ctx.guild.id)
        if not player.queue:
            return await self._embed_msg(ctx, "Nothing queued. (¬_¬ )")
        if dj_enabled:
            if not await self._can_instaskip(ctx, ctx.author):
                return await self._embed_msg(ctx, "You need the DJ role to remove songs. (¬_¬ )")
        if (
            (not ctx.author.voice or ctx.author.voice.channel != player.channel)
            and not await self._can_instaskip(ctx, ctx.author)
        ):
            return await self._embed_msg(
                ctx, "You must be in the voice channel to manage the queue. ╮(￣ω￣;)╭"
            )
        if index > len(player.queue) or index < 1:
            return await self._embed_msg(
    ...
    async def search(self, ctx, *, query):
        ...
        if (
            ...
            return await self._embed_msg(ctx, "You must be in the voice channel to enqueue songs, baka.")
        await self._data_check(ctx)
    ...
    async def seek(self, ctx, seconds: int = 30):
        ...
        if (
            ...
            return await self._embed_msg(ctx, "You must be in the voice channel to use seek. (・_・;)")
        if dj_enabled:
            if (
                not await self._can_instaskip(ctx, ctx.author)
                and not await self._is_alone(ctx, ctx.author)
            ):
                return await self._embed_msg(ctx, "You need the DJ role to use seek. (・_・;)")
        if player.current:
            if player.current.is_stream:
                return await self._embed_msg(ctx, "Can't seek on a stream. (・_・;)")
            else:
    ...
    async def skip(self, ctx):
        ...
        if not self._player_check(ctx):
            return await self._embed_msg(ctx, "Nothing playing. (￣ω￣;)")
        player = lavalink.get_player(ctx.guild.id)
        if (
            (not ctx.author.voice or ctx.author.voice.channel != player.channel)
            and not await self._can_instaskip(ctx, ctx.author)
            ...
            return await self._embed_msg(
                ctx, "You must be in the voice channel to skip the music. (￣ω￣;)"
            )
        dj_enabled = await self.config.guild(ctx.guild).dj_enabled()
        vote_enabled = await self.config.guild(ctx.guild).vote_enabled()
        ...
        if vote_enabled:
            if not await self._can_instaskip(ctx, ctx.author):
                ...
                if vote >= percent:
                    ...
                    await self._embed_msg(ctx, "Vote threshold met. Democracy rules!")
    ...
    async def _skip_action(ctx):
        ...
        if not player.queue:
            ...
            if player.current.is_stream:
                embed = discord.Embed(
                    colour=ctx.guild.me.top_role.colour, title="There's nothing in the queue. (•ิ_•ิ)?"
                )
    ...
    async def stop(self, ctx):
        ...
        vote_enabled = await self.config.guild(ctx.guild).vote_enabled()
        if not self._player_check(ctx):
            return await self._embed_msg(ctx, "There's nothing playing, baka")
        player = lavalink.get_player(ctx.guild.id)
        if (
            (not ctx.author.voice or ctx.author.voice.channel != player.channel)
            and not await self._can_instaskip(ctx, ctx.author)
            ...
            return await self._embed_msg(
                ctx, "You must be in the voice channel to stop the music, baka."
            )
        if vote_enabled or vote_enabled and dj_enabled:
            if (
                ...
                return await self._embed_msg(
                    ctx, "There are other people listening - vote to skip instead. Don't be mean >.<"
                )
        if dj_enabled and not vote_enabled:
            if not await self._can_instaskip(ctx, ctx.author):
                return await self._embed_msg(ctx, "You need the DJ role to stop the music. 	( ` ω ´ )")
    ...
    async def llsetup(self, ctx):
        """(Gtoyos Command). Lavalink server configuration options."""
        if ctx.invoked_subcommand is None:
            await ctx.send_help()

    @llsetup.command()
    async def external(self, ctx):
        """(Gtoyos Command). Toggles using external lavalink servers."""
    ...
    async def host(self, ctx, host):
        """(Gtoyos Command). Set the lavalink server host."""
    ...
    async def password(self, ctx, password):
        """(Gtoyos Command). Set the lavalink server password."""
    ...
    async def restport(self, ctx, rest_port):
        """(Gtoyos Command). Set the lavalink REST server port."""
    ...
    async def wsport(self, ctx, ws_port):
        """(Gtoyos Command). Set the lavalink websocket server port."""
