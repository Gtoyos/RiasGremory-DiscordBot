...
class Audio(commands.Cog):
    ...
    async def vote(self, ctx, percent: int):
        ...
        if percent == 0:
            enabled = False
            await self._embed_msg(
                ctx, _("Voting disabled. All users can use queue management commands. Anarchism rules!")
            )
        else:
            enabled = True
            await self._embed_msg(
    ...
    async def bump(self, ctx, index: int):
        ...
        if not self._player_check(ctx):
            return await self._embed_msg(ctx, _("Nothing playing. (-_-;)・・・"))
        player = lavalink.get_player(ctx.guild.id)
        if (
            not ctx.author.voice or ctx.author.voice.channel != player.channel
        ) and not await self._can_instaskip(ctx, ctx.author):
            return await self._embed_msg(
                ctx, _("You must be in the voice channel to bump a song. ^^")
            )
        if dj_enabled:
            if not await self._can_instaskip(ctx, ctx.author):
                return await self._embed_msg(ctx, _("You need the DJ role to bump songs. (*μ_μ)"))
        if index > len(player.queue) or index < 1:
            return await self._embed_msg(
                ctx, _("Song number must be greater than 1 and within the queue limit, baka.")
            )
    ...
    async def disconnect(self, ctx):
        ...
        if self._player_check(ctx):
            if dj_enabled:
                if not await self._can_instaskip(ctx, ctx.author):
                    return await self._embed_msg(ctx, _("There are other people listening to music. Don't be a bad person >:c )"))
    ...
    async def pause(self, ctx):
        ...
        if (
            ...
            return await self._embed_msg(
                ctx, _("You must be in the voice channel to pause the music, baka")
            )
        if dj_enabled:
            if not await self._can_instaskip(ctx, ctx.author) and not await self._is_alone(
                ctx, ctx.author
            ):
                return await self._embed_msg(ctx, _("You need the DJ role to pause tracks, baka"))
        ...
        if player.paused and command == "pause":
            return await self._embed_msg(ctx, _("Track is paused. (」°ロ°)」"))
        if player.current and command == "resume":
            return await self._embed_msg(ctx, _("Track is playing. (⌒‿⌒)"))
        await self._embed_msg(ctx, _("Nothing playing. (T_T)"))
    ...
    async def percent(self, ctx):
        """Queue percentage."""
        if not self._player_check(ctx):
            return await self._embed_msg(ctx, _("Nothing playing. (T_T)"))
        player = lavalink.get_player(ctx.guild.id)
        ...
        try:
            ...
            return await self._embed_msg(ctx, _("Nothing in the queue. 〣( ºΔº )〣"))
    ...
    async def play(self, ctx, *, query):
        ...
        if not self._player_check(ctx):
            try:
                ...
                return await self._embed_msg(ctx, _("Connect to a voice channel first, baka"))
        if dj_enabled:
            if not await self._can_instaskip(ctx, ctx.author):
        ...
        if (
            ...
            return await self._embed_msg(
                ctx, _("You must be in the voice channel to use the play command, baka")
            )
        ...
        if not tracks:
            return await self._embed_msg(ctx, _("Nothing found. (•ิ_•ิ)?"))
        ...
        if ("ytsearch:" or "localtrack") not in query and len(tracks) > 1:
            ...
            embed = discord.Embed(
                colour=await ctx.embed_colour(),
                title=_("Playlist Enqueued (ﾉ◕ヮ◕)ﾉ*:･ﾟ✧"),
    ...
    async def _playlist_append(self, ctx, playlist_name, *url):
        ...
        async with self.config.guild(ctx.guild).playlists() as playlists:
            try:
                ...
                return await self._embed_msg(ctx, _("No playlist with that name. :thonk:"))
    ...
    async def _playlist_create(self, ctx, playlist_name):
        ...
        async with self.config.guild(ctx.guild).playlists() as playlists:
            if playlist_name in playlists:
                return await self._embed_msg(
                    ctx, _("Playlist name already exists, try again with a different name. Be more creative!")
                )
        ...
        async with self.config.guild(ctx.guild).playlists() as playlists:
            try:
                if playlists[playlist_name][
                    ...
                    return await self._embed_msg(
                        ctx, _("You are not the author of that playlist, baka.")
                    )
                del playlists[playlist_name]
            except KeyError:
                return await self._embed_msg(ctx, _("No playlist with that name. o_o"))
    ...
    async def _playlist_info(self, ctx, playlist_name):
        ...
        try:
            ...
            return await self._embed_msg(ctx, _("No playlist with that name. (・・;)ゞ"))
    ...
    async def _playlist_queue(self, ctx, playlist_name=None):
        ...
        async with self.config.guild(ctx.guild).playlists() as playlists:
            ...
            if not self._player_check(ctx):
                return await self._embed_msg(ctx, _("Nothing playing.  	(・・;)ゞ"))
        player = lavalink.get_player(ctx.guild.id)
        tracklist = []
        ...
        if not playlist_name:
            await self._embed_msg(ctx, _("Please enter a name for this playlist. \(^ヮ^)/"))
        ...
        async with self.config.guild(ctx.guild).playlists() as playlists:
            try:
                if playlists[playlist_name][
                    ...
                    return await self._embed_msg(
                        ctx, _("You are not the author of that playlist. Don't touch what's not yours!")
                    )
            except KeyError:
                return await self._embed_msg(ctx, _("No playlist with that name. (•ิ_•ิ)?"))
            track_list = playlists[playlist_name]["tracks"]
    ...
    async def _playlist_start(self, ctx, playlist_name=None):
        ...
        try:
            ...
            await self._embed_msg(ctx, _("That playlist doesn't exist. ( ; ω ; )"))
    ...
    async def prev(self, ctx):
        ...
        if dj_enabled:
            if not await self._can_instaskip(ctx, ctx.author) and not await self._is_alone(
                ...
                return await self._embed_msg(ctx, _("You need the DJ role to skip tracks, baka"))
        if (
            not ctx.author.voice or ctx.author.voice.channel != player.channel
        ) and not await self._can_instaskip(ctx, ctx.author):
            return await self._embed_msg(
                ctx, _("You must be in the voice channel to skip the music, baka")
            )
        if shuffle:
    ...
    async def _build_queue_page(self, ctx, player, page_num):
        ...
        try:
            ...
            return await self._embed_msg(ctx, _("There's nothing in the queue. Add something!"))
        pos = lavalink.utils.format_time(player.position)
    ...
    async def _queue_search(self, ctx, *, search_words):
        ...
        if not search_list:
            return await self._embed_msg(ctx, _("No matches.  	(・_・ヾ"))
    ...
    async def remove(self, ctx, index: int):
        ...
        player = lavalink.get_player(ctx.guild.id)
        if not player.queue:
            return await self._embed_msg(ctx, _("Nothing queued. (〃￣ω￣〃ゞ"))
        if dj_enabled:
            if not await self._can_instaskip(ctx, ctx.author):
                return await self._embed_msg(ctx, _("You need the DJ role to remove tracks, baka."))
        if (
            not ctx.author.voice or ctx.author.voice.channel != player.channel
        ) and not await self._can_instaskip(ctx, ctx.author):
            return await self._embed_msg(
    ...
    async def search(self, ctx, *, query):
        ...
        if not self._player_check(ctx):
            try:
                if not ctx.author.voice.channel.permissions_for(ctx.me).connect or self._userlimit(
                    ...
                    return await self._embed_msg(
                        ctx, _("I don't have permission to connect to your channel.＼(º □ º l|l)/")
                    )
                await lavalink.connect(ctx.author.voice.channel)
                player = lavalink.get_player(ctx.guild.id)
                player.store("connect", datetime.datetime.utcnow())
        ...
        if not isinstance(query, list):
            ...
            if query.startswith("list ") or query.startswith("folder:"):
                ...
                if not tracks:
                    return await self._embed_msg(ctx, _("Nothing found.ヽ(ー_ー )ノ"))
    ...
    async def seek(self, ctx, seconds: int = 30):
        ...
        vote_enabled = await self.config.guild(ctx.guild).vote_enabled()
        if not self._player_check(ctx):
            return await self._embed_msg(ctx, _("Nothing playing. 	¯\_(ツ)_/¯"))
        player = lavalink.get_player(ctx.guild.id)
        if (
            not ctx.author.voice or ctx.author.voice.channel != player.channel
        ) and not await self._can_instaskip(ctx, ctx.author):
            return await self._embed_msg(ctx, _("You must be in the voice channel to use seek. 	¯\_(ツ)_/¯"))
        if dj_enabled:
            if not await self._can_instaskip(ctx, ctx.author) and not await self._is_alone(
                ctx, ctx.author
            ):
                return await self._embed_msg(ctx, _("You need the DJ role to use seek, baka"))
        if vote_enabled:
            if not await self._can_instaskip(ctx, ctx.author) and not await self._is_alone(
                ctx, ctx.author
            ):
                return await self._embed_msg(
                    ctx, _("There are other people listening - vote to skip instead. Don't be a jerk.")
                )
        if player.current:
            if player.current.is_stream:
                return await self._embed_msg(ctx, _("Sorry, I can't seek on a stream.(/ω＼)"))
            else:
        ...
        if vote_enabled:
            if not await self._can_instaskip(ctx, ctx.author):
                if ctx.author.id in self.skip_votes[ctx.message.guild]:
                    ...
                    reply = _("I removed your vote to skip.(´• ω •`)")
                else:
                    self.skip_votes[ctx.message.guild].append(ctx.author.id)
                    reply = _("You voted to skip.(⌒‿⌒)")
    ...
    async def _skip_action(self, ctx):
        ...
        if not player.queue:
            try:
                ...
                return await self._embed_msg(ctx, _("There's nothing in the queue. 	¯\_(ツ)_/¯"))
            time_remain = lavalink.utils.format_time(dur - pos)
            if player.current.is_stream:
                embed = discord.Embed(
                    colour=await ctx.embed_colour(), title=_("There's nothing in the queue. ¯\_(ツ)_/¯")
                )
                embed.set_footer(
    ...
    async def stop(self, ctx):
        ...
        if (
            ...
            return await self._embed_msg(
                ctx, _("You must be in the voice channel to stop the music,baka.")
            )
        ...
        if dj_enabled and not vote_enabled:
            if not await self._can_instaskip(ctx, ctx.author):
                return await self._embed_msg(ctx, _("You need the DJ role to stop the music, baka."))
